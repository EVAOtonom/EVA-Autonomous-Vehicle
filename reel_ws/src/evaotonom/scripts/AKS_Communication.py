...
class STM_Communication:
    ...
    def send_command(self, num_of_registers, data):
        try:
            ...
            if -32769 < data < 32768:
                if data < 0:
                    data = 65536 + data
                self.stm.write_register(num_of_registers.value, data, functioncode=6)
    ...
    def publish_data(self):
        gps_latitude = self.read_data(self.stm, Register.GPS_LATITUDE)
        gps_longitude = self.read_data(self.stm, Register.GPS_LONGITUDE)
        read_odometer = self.read_data(self.stm, Register.READ_ODOMETER)
        ...
        # Publishers
        self.gps_latitude_pub = rospy.Publisher('gps_latitude', Float32, queue_size=10)
        self.gps_longitude_pub = rospy.Publisher('gps_longitude', Float32, queue_size=10)
        self.read_odometer_pub = rospy.Publisher('read_odometer', Float32, queue_size=10)
