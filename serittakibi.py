...
def detect_image(image_path, yakinmi,model):
...
def serittakibi(cam,model,ensol,sol,sag,ensag,lidaretkin,goruntuislemeetkin):
    ...
    while True:
        yakinmi = 0
        while lidaretkin[0]==1 or goruntuislemeetkin[0]==1:
            pass
        ...
        if float((midpoints['sol'][1]) != 0 or float(midpoints['sol'][0]) != 0) and (float(midpoints['sag'][1]) != 0 or float(midpoints['sag'][0])) != 0:
            if yakinmi == 1:
            ...
            image = cv2.line(image, (int(image.shape[1] / 2), int(image.shape[0])),
                            (int(image.shape[1] / 2), int(orta_y)), (0, 255, 0),
                            2)
            image = cv2.line(image, (int(image.shape[1] / 2), int(image.shape[0])),
                            (int(orta_x), int(orta_y)), (0, 255, 0), 2)
            image = cv2.line(image, (int(image.shape[1] / 2), int(orta_y)),
                            (int(orta_x), int(orta_y)), (0, 255, 0), 2)
        ...
        donme_acisi = 2*int(donme_acisi * 53 / 10)
        """
        ...
        if donme_acisi > guvenaraligi:
            ...
            try:
                ...
                if uzaklik_x < 0:
                    ...
                    pid = "t-" + pid + "\n"
                    print(pid)
                    if lidaretkin[0]==0 and goruntuislemeetkin[0]==0:
                        write_read(pid)
